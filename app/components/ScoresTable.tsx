...
import CatalystLegend from "@/app/components/CatalystLegend";

/** ─── Tunables ─── */
const WEIGHTS = { rvol: 0.55, ai: 0.20, gap: 0.18, change: 0.07 };
const RVOL_TRADE = 3.0, RVOL_TRADE_FALLBACK = 4.5;
const GAP_MIN_TRADE = 4, GAP_MIN_TRADE_FALLBACK = 6;
const CHANGE_MIN_TRADE = 3, CHANGE_MIN_TRADE_FALLBACK = 5;
const RVOL_WATCH = 1.8, GAP_MIN_WATCH = 1.5, CHANGE_MIN_WATCH = 0;
const FLOAT_TARGET_M = 20, FLOAT_BONUS = 3, FLOAT_PENALTY_MAX = 12, FLOAT_PENALTY_CAP_M = 200;
const NEWS_WINDOW_MIN = 60, NEWS_BONUS = 8;

type ScoreRow = {
  ticker: string; price?: number | null; gap_pct?: number | null; change_pct?: number | null;
  rvol?: number | null; float_m?: number | null; ai_score?: number | null; rsi14m?: number | null; volume?: number | null; ts?: string | null;
  catalyst?: { recent: boolean; latestISO?: string | null; latestUrl?: string | null; latestHeadline?: string | null; latestTag?: string | null; };
  actionScore?: number; action?: "TRADE" | "WATCH" | "SKIP";
};
type ScoresPayload = { generatedAt: string | null; scores: ScoreRow[] };
type NewsItem = { ticker: string; headline?: string; url?: string; published?: string; tag?: string; };
...
const finvizNewsUrl = (t: string) => `https://finviz.com/quote.ashx?t=${encodeURIComponent(t)}#news`;
...
function mapFinvizStocksToScores(rows: any[]): ScoresPayload {
  const scores: ScoreRow[] = rows.map((s) => {
    ...
    if (floatM == null && s.float_shares != null) { const abs = safeNum(s.float_shares); if (abs != null) floatM = Math.round((abs / 1_000_000) * 10) / 10; }
    if (floatM == null && s.float != null) floatM = safeNum(s.float);
    return {
      ticker: t, price: safeNum(s.price), gap_pct: gap, change_pct: parseChange(s.changePercent ?? s.change_pct ?? s.change),
      rvol: relv, float_m: floatM, ai_score: null, rsi14m: null, volume: safeNum(s.volume), ts: s.lastUpdated || s.ts || null,
    };
...
export default function ScoresTable({ onTopTickersChange }: { onTopTickersChange?: (tickers: string[]) => void; }) {
  ...
  const [priceMin, setPriceMin] = useState(1), [priceMax, setPriceMax] = useState(5), [gapMin, setGapMin] = useState(5), [onlyStrong, setOnlyStrong] = useState(false);
  const [pageSize, setPageSize] = useState<number>(() => { if (typeof window === "undefined") return 10; const saved = Number(localStorage.getItem("sf_page_size") || "10"); return [10,25,50].includes(saved) ? saved : 10; });
  useEffect(() => { try { localStorage.setItem("sf_page_size", String(pageSize)); } catch {} }, [pageSize]);
  const [page, setPage] = useState(1);

  async function loadOnce() {
    let finvizRows: any[] = []; let aiMap: Record<string, any> = {}; let aiGenerated: string | null = null;
    const newsMap = new Map<string, { latestISO: string | null; latestUrl: string | null; recent: boolean; latestHeadline: string | null; latestTag: string | null; }>();

    // 1) stocks
    try { const res = await fetch("/api/stocks", { cache: "no-store" }); if (res.ok) { const j = await res.json(); if (j?.success && Array.isArray(j.data)) finvizRows = j.data; } } catch {}
    // 2) AI scores
    try { const res = await fetch("/api/scores", { cache: "no-store" }); if (res.ok) { const j = await res.json(); (j?.scores || []).forEach((s: any) => { const t = String(s.ticker || "").toUpperCase(); if (t) aiMap[t] = s; }); aiGenerated = j?.generatedAt ?? null; } } catch {}
    // 3) news cache
    try {
      const res = await fetch("/api/news", { cache: "no-store" });
      if (res.ok) {
        const j = (await res.json()) as NewsPayload;
        const now = Date.now(); const byT: Record<string, NewsItem[]> = {};
        (j?.items || []).forEach((n) => { const t = String(n.ticker || "").toUpperCase(); if (!t) return; (byT[t] ??= []).push(n); });
        for (const [t, arr] of Object.entries(byT)) {
          arr.sort((a, b) => (parseNewsTime(b.published) ?? 0) - (parseNewsTime(a.published) ?? 0));
          const latest = arr[0]; const ms = parseNewsTime(latest?.published); const recent = ms != null ? (now - ms) / 60000 <= NEWS_WINDOW_MIN : false;
          newsMap.set(t, { latestISO: ms != null ? new Date(ms).toISOString() : null, latestUrl: latest?.url ?? null, recent, latestHeadline: latest?.headline ?? null, latestTag: latest?.tag ?? null });
        }
      }
    } catch {}
    ...
    if (finvizRows.length) {
      ...
      payload.scores = payload.scores.map((row) => {
        ...
        row.catalyst = newsMap.get(row.ticker) || { recent: false, latestISO: null, latestUrl: null, latestHeadline: null, latestTag: null };
        const { score, action } = computeScoreAndDecision(row);
        row.actionScore = score; row.action = action;
        return row;
      });
      payload.generatedAt = payload.generatedAt || aiGenerated || null;
      setData(payload); return;
    }
    setData({ generatedAt: aiGenerated, scores: [] });
  }

  useEffect(() => { loadOnce(); const id = setInterval(loadOnce, 60_000); return () => clearInterval(id); }, []);
  ...
  const filteredAll = useMemo(() => {
    return (data.scores || [])
      ...
      .filter((r) => {
        ...
        const ai = r.ai_score ?? 0; const rv = (r.rvol ?? 0); const as = r.actionScore ?? 0;
  ...
  const start = (page - 1) * pageSize, end = Math.min(filteredAll.length, start + pageSize);
  const visible = filteredAll.slice(start, end);

  // tell NewsPanel which tickers are visible
  const tickRef = useRef<string>("");
  useEffect(() => { const top = visible.map((r) => r.ticker); const key = top.join(","); if (key !== tickRef.current) { tickRef.current = key; onTopTickersChange?.(top); } }, [visible, onTopTickersChange]);
  ...
  return (
    <section className="rounded-2xl bg-white/5 border border-white/10 p-4">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="ml-auto text-sm opacity-80">Avg Gap: {avgGap ? avgGap.toFixed(1) + "%" : "—"} • Results: {filteredAll.length}</div>
      </div>
      ...
      {/* Legend chip (no extra grid width) */}
      <div className="mt-3">
        <CatalystLegend />
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="text-white/90">
            {visible.map((r) => {
              ...
              const tag = r.catalyst?.latestTag || undefined;
              const finvizLink = finvizNewsUrl(r.ticker);

              return (
                <tr key={r.ticker} className="border-top border-white/10" style={{ background: bg, boxShadow: leftAccent as any }}>
                  <Td>{r.ticker}</Td>
                  ...
                  <TdR>
                    {hasNews ? (
                      <a href={finvizLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 underline underline-offset-2 decoration-white/30">
                        <CatalystIcon tag={tag} />
                        <span className="text-xs text-white/80">{timeOnly(r.catalyst?.latestISO)}</span>
                      </a>
      ...
      <div className="mt-3 flex items-center justify-between text-sm">
        <div className="opacity-80">Showing {visible.length ? `${start + 1}–${end}` : "0"} of {filteredAll.length}</div>
        <div className="flex items-center gap-2">
          <button className="px-3 py-1.5 rounded-lg border border-white/15 bg-white/5 hover:bg-white/10 disabled:opacity-40" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}>Prev</button>
          <span>Page {page} / {Math.max(1, Math.ceil(filteredAll.length / pageSize))}</span>
          <button className="px-3 py-1.5 rounded-lg border border-white/15 bg-white/5 hover:bg-white/10 disabled:opacity-40" onClick={() => setPage((p) => Math.min(Math.ceil(filteredAll.length / pageSize), p + 1))} disabled={page >= Math.ceil(filteredAll.length / pageSize)}>Next</button>
        </div>
      </div>
    </section>
  );
...
/** Scoring + Decision (with rVol fallback) */
function computeScoreAndDecision(r: ScoreRow): { score: number; action: "TRADE" | "WATCH" | "SKIP" } {
  ...
  const rvRaw = (providedRvol != null && providedRvol > 0) ? providedRvol : (gapVal >= 40 ? 4.5 : gapVal >= 20 ? 3.0 : 1.0);
  ...
  if (f != null) { if (f <= FLOAT_TARGET_M) score += FLOAT_BONUS; else { const capped = Math.min(f, FLOAT_PENALTY_CAP_M); const frac = (capped - FLOAT_TARGET_M) / (FLOAT_PENALTY_CAP_M - FLOAT_TARGET_M); score -= FLOAT_PENALTY_MAX * clamp(frac, 0, 1); } }
  const hasNews = !!r.catalyst?.latestISO; if (hasNews) score += NEWS_BONUS;
  if (chgVal < 0) score -= 4;
  const rsi = r.rsi14m ?? null; if (rsi != null && (rsi >= 85 || rsi <= 15)) score -= 4;
  score = clamp(score, 0, 100);
  let action: "TRADE" | "WATCH" | "SKIP";
  if ((hasNews && rvRaw >= RVOL_TRADE && gapVal >= GAP_MIN_TRADE && chgVal >= CHANGE_MIN_TRADE) || (!hasNews && rvRaw >= RVOL_TRADE_FALLBACK && gapVal >= GAP_MIN_TRADE_FALLBACK && chgVal >= CHANGE_MIN_TRADE_FALLBACK)) action = "TRADE";
